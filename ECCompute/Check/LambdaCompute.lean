...
/-- Fully `Nat` mirror of `lambdaComputeBool`. The arguments are `a₂ = c2p - c2m`,
`a₄ = c4p - c4m`, the modulus `p`, its quadratic-residue mask `qmask`, the label residue `tval` for
`θ`, and the point coordinate `x = (xp - xm) / xden`. For `qmask = qrMask p` (`p` an odd prime) it
agrees with `lambdaComputeBool` (see `lambdaComputeBoolNatMask_eq`). -/
