...
/-- `bId` turns `Bool.xor` into addition in `ZMod 2`. -/
...
/-- `checkInvRow` peels the head column of `M`, advancing the column index. -/
...
/-- `checkInvGo` peels the head row of `B`, advancing the row index. -/
...
/-- `Nat.one_shiftLeft` restated with the primitive `Nat.shiftLeft` head that `maskBelow` uses, so
that `simp only` matches it. -/
