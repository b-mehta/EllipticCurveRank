...
computed descent character; `checkB_true` recovers the individual entry equalities.

## Main definitions

* `ECCompute.toLabN`: the `Nat` label triples the kernel-reduced checker consumes.
* `ECCompute.checkB`: the aggregate check.

## Main results

* `ECCompute.checkB_true`: every entry of `matB` is the descent character of its label at its point.
-/
...
/-- One row of the descent-matrix check. The coefficient `a₂` arrives as the difference of naturals
`c2p - c2m`, `a₄` as `c4p - c4m`, and the point's `x`-coordinate as `(xnp - xnm) / xden`. Folds over
the `Nat` label triples `labN = (p, tval, q)`, consuming the row bitmask `b` one bit at a time and
comparing each bit against the mask-based `Nat`-valued descent character. -/
...
/-- `checkBRow` peels the head label, consuming the low bit of the row bitmask. -/
private theorem checkBRow_cons (c2p c2m c4p c4m xnp xnm xden b : ℕ) (l : ℕ × ℕ × ℕ)
    (ls : List (ℕ × ℕ × ℕ)) :
...
/-- Fold over the rows, pairing each row bitmask of `matB` with its point in `pt` and checking the
row with `checkBRow`. Here `a₂ = c2p - c2m` and `a₄ = c4p - c4m`, and each point's numerator is
split into `x.num.toNat - (-x.num).toNat` over the denominator `x.den`. -/
...
/-- `checkBGo` peels the head row of `matB` together with its point. -/
private theorem checkBGo_cons_cons (c2p c2m c4p c4m : ℕ) (labN : List (ℕ × ℕ × ℕ)) (b : ℕ)
    (bs : List ℕ) (p : ℚ × ℚ) (ps : List (ℚ × ℚ)) :
...
/-- The aggregate descent-matrix check: `true` iff every supplied mask is `qrMask` of its label's
prime and every bit of `matB` is the descent character of that label at that point. Spec:
`checkB_true`. -/
...
private theorem checkBGo_row {c2p c2m c4p c4m : ℕ} {labN : List (ℕ × ℕ × ℕ)} :
...
theorem checkB_true {a₂ a₄ : ℤ} {matB : List ℕ} {rho : ℕ}
  ...
  have hbridge : lambdaComputeBoolNatMask a₂.toNat (-a₂).toNat a₄.toNat (-a₄).toNat
    ...
    lambdaComputeBoolNatMask_eq a₂ a₄ L.1 (L.2 : ZMod L.1) P.1 _ _ _ _ _ _ _ _
