...
equation `y² + a₁xy + a₃y = x³ + a₂x² + a₄x + a₆` holds in `ℚ`.

## Main definitions

* `ECCompute.checkPoint`: the check for one point.
* `ECCompute.checkPoints`: the check across a list of points.

## Main results

* `ECCompute.checkPoint_iff`, `ECCompute.checkPoints_iff`: each check is `true` exactly when the
  affine Weierstrass equation holds.
-/
...
/-- Kernel-reducible point-on-curve check: `true` exactly when `(x, y)` satisfies the Weierstrass
equation of `⟨a₁, a₂, a₃, a₄, a₆⟩` over `ℚ`. Spec: `checkPoint_iff`. -/
...
/-- The kernel-reducible checker `checkPoint` returns `true` if and only if the point `(x, y)`
satisfies the affine Weierstrass equation of the model `⟨a₁, a₂, a₃, a₄, a₆⟩`. -/
theorem checkPoint_iff (a₁ a₂ a₃ a₄ a₆ : ℤ) (x y : ℚ) :
  ...
  simp only [WeierstrassCurve.Affine.equation_iff, checkPoint, Int.beq'_eq, hmul, hadd]
...
example : checkPoint 1 0 1 0 0 0 0 = true := rfl
...
example : (⟨1, 0, 1, 0, 0⟩ : WeierstrassCurve ℚ).toAffine.Equation 0 0 :=
  (checkPoint_iff 1 0 1 0 0 0 0).mp rfl
