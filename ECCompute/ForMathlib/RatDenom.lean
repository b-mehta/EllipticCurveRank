...
variable (hp : p.Prime) {x y : ℚ}
include hp

/-- `(·.den : ZMod p) ≠ 0` is closed under addition. -/
public theorem den_add_ne_zero (hx : (x.den : ZMod p) ≠ 0) (hy : (y.den : ZMod p) ≠ 0) :
    ((x + y).den : ZMod p) ≠ 0 :=
...
public theorem den_sub_ne_zero (hx : (x.den : ZMod p) ≠ 0) (hy : (y.den : ZMod p) ≠ 0) :
    ((x - y).den : ZMod p) ≠ 0 :=
  have : Fact p.Prime := ⟨hp⟩
...
public theorem den_mul_ne_zero (hx : (x.den : ZMod p) ≠ 0) (hy : (y.den : ZMod p) ≠ 0) :
    ((x * y).den : ZMod p) ≠ 0 :=
  have : Fact p.Prime := ⟨hp⟩
...
public theorem den_div_ne_zero {a b : ℚ} (hb : (b.den : ZMod p) ≠ 0)
