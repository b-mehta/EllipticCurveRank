...
`checkInvBS ρ g rep Mstack B` returns `true` (and `B` has length `ρ`), the matrix `toMat B ρ`
over `𝔽₂` is a unit. The per-row selectors are built in-kernel from `B` (`bspreadK`), and `Mstack`
decodes to a right inverse `N` with `B · N = I` over `𝔽₂`; the aligned-window column-parity
extraction is unproved here. -/
public theorem checkInvBS_isUnit {ρ g rep Mstack : ℕ}
    (hBlen : B.length = ρ) (h : checkInvBS ρ g rep Mstack B) : IsUnit (toMat B ρ) := by
  sorry
