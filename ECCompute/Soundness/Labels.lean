...
the descent lemma packaged as `ECCompute.DescentHyp`:

* `p ∤ 6`     (so `p ≠ 2, 3`);
* `p ∤ Δ`     where `Δ` is the integer discriminant of `y² = x³ + a₂x² + a₄x + a₆`;
* `f(θ) ≡ 0 (mod p)`.

The kernel-reducible checks `ECCompute.checkLabel` / `ECCompute.checkLabels` live in
`ECCompute.Kernel`; this file proves them sound: `descentHyp_of_checkLabel` turns
`checkLabel … = true` (with a separately supplied primality proof) into a `DescentHyp`, and
`checkLabels_true` lifts a passing `checkLabels` to every label.

## Main declarations

* `ECCompute.discrInt`: the integer discriminant of `curve a₂ a₄ a₆`.
* `ECCompute.curve_Δ_num`: `(curve …).Δ.num = discrInt …`.
* `ECCompute.descentHyp_of_checkLabel`: the passage from `checkLabel` to `DescentHyp`.
-/
