...
public theorem intResNat_cast {p : ℕ} {z : ℤ} (hp : p ≠ 0) : ((z % p).toNat : ZMod p) = z := by
...
public theorem no_rat_root_of_monicHasNoRootMod {b c : ℤ} (hℓ : 1 < ℓ)
  ...
  exact no_int_root_of_monicHasNoRootMod hℓ h z
    (mod_cast (by simp only [polyEval]; grind : (polyEval [c, b, 1] z : ℚ) = 0))
