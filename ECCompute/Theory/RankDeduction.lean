...
* `ECCompute.RankDeduction.natCard_modN_two`: the cardinality identity
  `Nat.card (ModN H 2) = 2 ^ finrank ℤ H * Nat.card H[2]`.
* `ECCompute.RankDeduction.rho_le_finrank_modN_two`: if `ρ` elements have `𝔽₂`-linearly
  independent images under some additive hom `φ : H →+ (Fin ρ → ZMod 2)`, then
  `ρ ≤ dim_{𝔽₂}(H ⧸ 2H)`.
* `ECCompute.RankDeduction.rank_ge_le`: the packaged deduction, `ρ ≤ rank H + t` whenever
  `Nat.card H[2] ≤ 2 ^ t` and there are `ρ` independent descent-character values.
...
namespace ECCompute.RankDeduction
...
/-- `H ⧸ 2H` is finite for a finitely generated `H`, being finitely generated and torsion. -/
...
/-- `H ⧸ 2H` is a finite-dimensional `𝔽₂`-vector space. -/
instance instFiniteDimModN [Module.Finite ℤ H] : Module.Finite (ZMod 2) (ModN H 2) :=
  Module.Finite.of_finite

/-- The 2-torsion of a finitely generated `H` is finite. -/
