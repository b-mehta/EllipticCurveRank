...
public theorem hasRankGE_of_certificate {a₁ a₂ a₃ a₄ a₆ : ℤ} (c : Certificate)
  ...
  have key : HasRankGE (curve c.a₂ c.a₄ c.a₆) (c.ρ - c.t) := by
    classical
    set pt : Fin c.ρ → ℚ × ℚ := fun i ↦ c.points[i]
    set ls : Fin c.ρ → ℕ × ℤ := fun j ↦ c.labels[j]
    have hEqn : ∀ i, (curve c.a₂ c.a₄ c.a₆).toAffine.Equation (pt i).1 (pt i).2 :=
      fun i ↦ hpt _ (List.getElem_mem _)
    ...
    have hns (i) : (curve c.a₂ c.a₄ c.a₆).toAffine.Nonsingular (pt i).1 (pt i).2 :=
      (Affine.equation_iff_nonsingular_of_Δ_ne_zero hΔ).mp (hEqn i)
