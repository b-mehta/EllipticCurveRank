...
@[expose] public def curve083 : WeierstrassCurve ℚ := ⟨0, 1, 1, -1856500, 1072474760⟩

/-- ICARM leaderboard curve 83 has Mordell-Weil rank at least `10`. -/
public theorem curve083_hasRankGE_10 : HasRankGE curve083 10 := by
  unfold curve083
  certify_curve torsion 7 "data/curve083.txt" "data/curve083-labels.txt"

/-- Curve 83 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve083.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 83. -/
public theorem curve083_j : curve083.j = -707634187818526550020096 / 87950374485438204043 :=
