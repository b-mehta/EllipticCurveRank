...
@[expose] public def curve046 : WeierstrassCurve ℚ := ⟨0, 0, 1, 0, -489468383913472842641289697078⟩

/-- ICARM leaderboard curve 46 has Mordell-Weil rank at least `16`. -/
public theorem curve046_hasRankGE_16 : HasRankGE curve046 16 := by
  unfold curve046
  certify_curve torsion 7 "data/curve046.txt" "data/curve046-labels.txt"

/-- Curve 46 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve046.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 46. -/
public theorem curve046_j : curve046.j = 0 := j_eq_iff.mpr (by decide +kernel)
