...
@[expose] public def curve016 : WeierstrassCurve ℚ := ⟨0, -1, 0, -23308665, 43082703225⟩

/-- ICARM leaderboard curve 16 has Mordell-Weil rank at least `6`. -/
public theorem curve016_hasRankGE_6 : HasRankGE curve016 6 := by
  unfold curve016
  certify_curve oneTorsion 11820 11 "data/curve016.txt" "data/curve016-labels.txt"

/-- Curve 16 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve016.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 16. -/
public theorem curve016_j : curve016.j = 341913290879033644447936 / 2174505011398265625 :=
