...
@[expose] public def curve002 : WeierstrassCurve ℚ := ⟨0, 1, 1, -2, 0⟩

/-- ICARM leaderboard curve 2 has Mordell-Weil rank at least `2`. -/
public theorem curve002_hasRankGE_2 : HasRankGE curve002 2 := by
  unfold curve002
  certify_curve torsion 5 "data/curve002.txt" "data/curve002-labels.txt"

/-- Curve 2 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve002.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 2. -/
public theorem curve002_j : curve002.j = 1404928 / 389 := j_eq_iff.mpr (by decide +kernel)
