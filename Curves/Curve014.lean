...
@[expose] public def curve014 : WeierstrassCurve ℚ := ⟨0, -1, 0, -24649, 1355209⟩

/-- ICARM leaderboard curve 14 has Mordell-Weil rank at least `4`, with full rational
`2`-torsion. -/
public theorem curve014_hasRankGE_4 : HasRankGE curve014 4 := by
  unfold curve014
  certify_curve fullTorsion "data/curve014.txt" "data/curve014-labels.txt"

/-- Curve 14 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve014.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 14. -/
public theorem curve014_j : curve014.j = 404370344147392 / 42649271289 :=
