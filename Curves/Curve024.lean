...
@[expose] public def curve024 : WeierstrassCurve ℚ := ⟨0, -1, 0, -2388994756, 44907381038500⟩

/-- ICARM leaderboard curve 24 has Mordell-Weil rank at least `6`. -/
public theorem curve024_hasRankGE_6 : HasRankGE curve024 6 := by
  unfold curve024
  certify_curve oneTorsion 110212 29 "data/curve024.txt" "data/curve024-labels.txt"

/-- Curve 24 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve024.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

set_option linter.style.longLine false in
/-- The `j`-invariant of curve 24. -/
public theorem curve024_j : curve024.j = 5890190437726738405327986193744 / 5660243641291976370659025 :=
