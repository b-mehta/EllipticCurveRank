...
@[expose] public def curve31 : WeierstrassCurve ℚ :=
  ⟨1, 0, 0, -2060795223235870670, 1037860392073475481628988676⟩

/-- ICARM leaderboard curve 31 has Mordell-Weil rank at least `10`. -/
public theorem curve31_hasRankGE_10 : HasRankGE curve31 10 := by
  unfold curve31
  certify_curve oneTorsion 4091012848 5 "data/curve31.txt" "data/curve31-labels.txt"

/-- Curve 31 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve31.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

set_option linter.style.longLine false in
/-- The `j`-invariant of curve 31. -/
public theorem curve31_j : curve31.j = 6501306190453387368858073830062529529671660018234764453 / 636725534658833049089877256511514677428219248562176 := j_eq_iff.mpr (by decide +kernel)
