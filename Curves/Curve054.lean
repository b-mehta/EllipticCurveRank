...
@[expose] public def curve054 : WeierstrassCurve ℚ := ⟨1, -1, 0, -79, 289⟩

/-- ICARM leaderboard curve 54 has Mordell-Weil rank at least `4`. -/
public theorem curve054_hasRankGE_4 : HasRankGE curve054 4 := by
  unfold curve054
  certify_curve torsion 7 "data/curve054.txt" "data/curve054-labels.txt"

/-- Curve 54 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve054.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 54. -/
public theorem curve054_j : curve054.j = 54915331401 / 468892 := j_eq_iff.mpr (by decide +kernel)
