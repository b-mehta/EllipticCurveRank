...
transported to the integral short model, are in `data/curve68.txt`; descent labels are in
`data/curve68-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
