...
@[expose] public def curve049 : WeierstrassCurve ℚ := ⟨0, 0, 1, -16312387, 25970162646⟩

/-- ICARM leaderboard curve 49 has Mordell-Weil rank at least `10`. -/
public theorem curve049_hasRankGE_10 : HasRankGE curve049 10 := by
  unfold curve049
  certify_curve torsion 7 "data/curve049.txt" "data/curve049-labels.txt"

/-- Curve 49 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve049.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 49. -/
public theorem curve049_j : curve049.j = -480038710884898894251134976 / 13561938370754827085483 :=
