...
@[expose] public def curve018 : WeierstrassCurve ℚ := ⟨0, -1, 0, -2069247973, 36191779888342⟩

/-- ICARM leaderboard curve 18 has Mordell-Weil rank at least `6`. -/
public theorem curve018_hasRankGE_6 : HasRankGE curve018 6 := by
  unfold curve018
  certify_curve oneTorsion 107848 29 "data/curve018.txt" "data/curve018-labels.txt"

/-- Curve 18 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve018.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 18. -/
public theorem curve018_j : curve018.j = 1428358461822810424179949568 / 1769156629341263370351 :=
