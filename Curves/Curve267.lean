...
transported to the integral short model, are in `data/curve267.txt`; descent labels are in
`data/curve267-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
...
public theorem curve267_j : curve267.j = 6368617184256 / 3674995477 := j_eq_iff.mpr (by decide +kernel)
