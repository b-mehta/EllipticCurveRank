...
@[expose] public def curve068 : WeierstrassCurve ℚ := ⟨0, 0, 0, 0, 46974552981863676115647417⟩

/-- ICARM leaderboard curve 68 has Mordell-Weil rank at least `15`. -/
public theorem curve068_hasRankGE_15 : HasRankGE curve068 15 := by
  unfold curve068
  certify_curve torsion 7 "data/curve068.txt" "data/curve068-labels.txt"

/-- Curve 68 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve068.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 68. -/
public theorem curve068_j : curve068.j = 0 := j_eq_iff.mpr (by decide +kernel)
