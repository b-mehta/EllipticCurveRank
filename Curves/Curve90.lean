...
transported to the integral short model, are in `data/curve90.txt`; descent labels are in
`data/curve90-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
...
public theorem curve90_j : curve90.j = 175432885286275280828360086171364125801928024075457989071778599235917469160397181641 / 14372991457346687754879118051280747837393220166817880670919159128474083891609600 := j_eq_iff.mpr (by decide +kernel)
