...
@[expose] public def curve020 : WeierstrassCurve ℚ := ⟨0, -1, 0, -122985156, 232744673700⟩

/-- ICARM leaderboard curve 20 has Mordell-Weil rank at least `6`. -/
public theorem curve020_hasRankGE_6 : HasRankGE curve020 6 := by
  unfold curve020
  certify_curve oneTorsion 7812 31 "data/curve020.txt" "data/curve020-labels.txt"

/-- Curve 20 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve020.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 20. -/
public theorem curve020_j : curve020.j = 803603536618091542566827344 / 373668526980316722179025 :=
