...
transported to the integral short model, are in `data/curve55.txt`; descent labels are in
`data/curve55-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
