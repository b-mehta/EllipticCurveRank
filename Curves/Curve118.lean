...
transported to the integral short model, are in `data/curve118.txt`; descent labels are in
`data/curve118-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
...
public theorem curve118_j : curve118.j = 77701427048448 / 8796007189 := j_eq_iff.mpr (by decide +kernel)
