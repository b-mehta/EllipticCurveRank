...
@[expose] public def curve060 : WeierstrassCurve ℚ := ⟨1, -1, 0, -1536664, 648294124⟩

/-- ICARM leaderboard curve 60 has Mordell-Weil rank at least `10`. -/
public theorem curve060_hasRankGE_10 : HasRankGE curve060 10 := by
  unfold curve060
  certify_curve torsion 13 "data/curve060.txt" "data/curve060-labels.txt"

/-- Curve 60 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve060.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 60. -/
public theorem curve060_j : curve060.j = 401292111107210344394841 / 50881111474471687972 :=
