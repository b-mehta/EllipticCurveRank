...
@[expose] public def curve023 : WeierstrassCurve ℚ := ⟨0, -1, 0, -1980651076, 33887665543876⟩

/-- ICARM leaderboard curve 23 has Mordell-Weil rank at least `6`. -/
public theorem curve023_hasRankGE_6 : HasRankGE curve023 6 := by
  unfold curve023
  certify_curve oneTorsion 99844 29 "data/curve023.txt" "data/curve023-labels.txt"

/-- Curve 23 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve023.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

set_option linter.style.longLine false in
/-- The `j`-invariant of curve 23. -/
public theorem curve023_j : curve023.j = 3356662445517276992391469247824 / 4707582391854002335700025 :=
