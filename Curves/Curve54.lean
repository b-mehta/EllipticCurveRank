...
transported to the integral short model, are in `data/curve54.txt`; descent labels are in
`data/curve54-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
