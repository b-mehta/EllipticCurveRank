...
@[expose] public def curve044 : WeierstrassCurve ℚ := ⟨0, 0, 0, -10012, 346900⟩

/-- ICARM leaderboard curve 44 has Mordell-Weil rank at least `7`. -/
public theorem curve044_hasRankGE_7 : HasRankGE curve044 7 := by
  unfold curve044
  certify_curve torsion 23 "data/curve044.txt" "data/curve044-labels.txt"

/-- Curve 44 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve044.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 44. -/
public theorem curve044_j : curve044.j = 433557066986496 / 47827988557 :=
