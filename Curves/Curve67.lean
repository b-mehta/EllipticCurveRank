...
over `ℚ`. It has Mordell-Weil rank at least `27`. The witness points from the leaderboard,
transported to the integral short model, are in `data/curve67.txt`; descent labels are in
`data/curve67-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
...
@[expose] public def curve67 : WeierstrassCurve ℚ :=
...
public theorem curve67_hasRankGE_27 : HasRankGE curve67 27 := by
  unfold curve67
  certify_curve torsion 47 "data/curve67.txt" "data/curve67-labels.txt"

/-- Curve 67 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve67.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

set_option linter.style.longLine false in
/-- The `j`-invariant of curve 67. -/
public theorem curve67_j : curve67.j = -19081574911308163519167857789930910431513370348120735399409672042058680390500520014879931412709582756211224505999425328544286863936085602605756678108056481 / 292295992467036730780372591065803086322380999542983847983425194363595329691796423905092278238635719085287829796511182852120658943291432879857572200448 :=
