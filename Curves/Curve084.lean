...
@[expose] public def curve084 : WeierstrassCurve ℚ := ⟨0, 0, 1, -2438527, 1545098346⟩

/-- ICARM leaderboard curve 84 has Mordell-Weil rank at least `10`. -/
public theorem curve084_hasRankGE_10 : HasRankGE curve084 10 := by
  unfold curve084
  certify_curve torsion 7 "data/curve084.txt" "data/curve084-labels.txt"

/-- Curve 84 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve084.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 84. -/
public theorem curve084_j : curve084.j = -1603638291915355209486336 / 103294665688000244363 :=
