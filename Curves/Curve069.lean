...
@[expose] public def curve069 : WeierstrassCurve ℚ := ⟨1, 0, 0, 0, 1⟩

/-- ICARM leaderboard curve 69 has Mordell-Weil rank at least `2`. -/
public theorem curve069_hasRankGE_2 : HasRankGE curve069 2 := by
  unfold curve069
  certify_curve torsion 7 "data/curve069.txt" "data/curve069-labels.txt"

/-- Curve 69 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve069.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 69. -/
public theorem curve069_j : curve069.j = -1 / 433 := j_eq_iff.mpr (by decide +kernel)
