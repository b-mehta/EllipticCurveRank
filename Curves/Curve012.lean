...
Leaderboard is
  ...
  `a₆ = 5525805855134237647573669959111819182152106703253507960837240477914941327771`
  `     6173425636721497`

over `ℚ`. It has Mordell-Weil rank at least `29`. Submitted to the leaderboard by David Renshaw.
-/
...
@[expose] public def curve012 : WeierstrassCurve ℚ :=
...
public theorem curve012_hasRankGE_29 : HasRankGE curve012 29 := by
  unfold curve012
  certify_curve torsion 67 "data/curve012.txt" "data/curve012-labels.txt"

/-- Curve 12 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve012.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

set_option linter.style.longLine false in
/-- The `j`-invariant of curve 12. -/
public theorem curve012_j : curve012.j = -2178278186417661901253576189103582365997935396472623778840162578414756885078160008166998745534482953456950649575682874999212260264866779577866068298184392291373758903459432104260760768986641 / 58514056884179895803252795545068205623215359270190478217393508483140281138663769634024489915971980794469429246265449489823479442097938729025702174957426421151737866434992604323840000000 :=
