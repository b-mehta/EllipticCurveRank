...
@[expose] public def curve051 : WeierstrassCurve ℚ := ⟨0, 0, 0, 90389647280869401176648335, 0⟩

/-- ICARM leaderboard curve 51 has Mordell-Weil rank at least `13`. -/
public theorem curve051_hasRankGE_13 : HasRankGE curve051 13 := by
  unfold curve051
  certify_curve oneTorsion 0 11 "data/curve051.txt" "data/curve051-labels.txt"

/-- Curve 51 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve051.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 51. -/
public theorem curve051_j : curve051.j = 1728 := j_eq_iff.mpr (by decide +kernel)
