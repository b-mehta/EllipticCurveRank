...
@[expose] public def curve047 : WeierstrassCurve ℚ := ⟨1, -1, 0, -106384, 13075804⟩

/-- ICARM leaderboard curve 47 has Mordell-Weil rank at least `8`. -/
public theorem curve047_hasRankGE_8 : HasRankGE curve047 8 := by
  unfold curve047
  certify_curve torsion 53 "data/curve047.txt" "data/curve047-labels.txt"

/-- Curve 47 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve047.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 47. -/
public theorem curve047_j : curve047.j = 133154226240373724121 / 3495093928855732 :=
