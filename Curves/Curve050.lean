...
@[expose] public def curve050 : WeierstrassCurve ℚ := ⟨0, 0, 1, -16359067, 26274178986⟩

/-- ICARM leaderboard curve 50 has Mordell-Weil rank at least `11`. -/
public theorem curve050_hasRankGE_11 : HasRankGE curve050 11 := by
  unfold curve050
  certify_curve torsion 17 "data/curve050.txt" "data/curve050-labels.txt"

/-- Curve 50 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve050.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 50. -/
public theorem curve050_j : curve050.j = -484171593245878168582557696 / 18031737725935636520843 :=
