...
@[expose] public def curve053 : WeierstrassCurve ℚ := ⟨0, 0, 1, -7, 6⟩

/-- ICARM leaderboard curve 53 has Mordell-Weil rank at least `3`. -/
public theorem curve053_hasRankGE_3 : HasRankGE curve053 3 := by
  unfold curve053
  certify_curve torsion 19 "data/curve053.txt" "data/curve053-labels.txt"

/-- Curve 53 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve053.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 53. -/
public theorem curve053_j : curve053.j = 37933056 / 5077 := j_eq_iff.mpr (by decide +kernel)
