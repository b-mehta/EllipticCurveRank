...
@[expose] public def curve019 : WeierstrassCurve ℚ := ⟨0, -1, 0, -3272444581, 72023792282806⟩

/-- ICARM leaderboard curve 19 has Mordell-Weil rank at least `6`. -/
public theorem curve019_hasRankGE_6 : HasRankGE curve019 6 := by
  unfold curve019
  certify_curve oneTorsion 134344 31 "data/curve019.txt" "data/curve019-labels.txt"

/-- Curve 19 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve019.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 19. -/
public theorem curve019_j : curve019.j = 35315050222442294737770643456 / 17606837538436654482975 :=
