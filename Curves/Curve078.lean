...
@[expose] public def curve078 : WeierstrassCurve ℚ := ⟨1, -1, 0, -201814, 34925104⟩

/-- ICARM leaderboard curve 78 has Mordell-Weil rank at least `8`. -/
public theorem curve078_hasRankGE_8 : HasRankGE curve078 8 := by
  unfold curve078
  certify_curve torsion 13 "data/curve078.txt" "data/curve078-labels.txt"

/-- Curve 78 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve078.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 78. -/
public theorem curve078_j : curve078.j = 909031208520136752441 / 643509175703572 :=
