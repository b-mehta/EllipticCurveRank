...
@[expose] public def curve062 : WeierstrassCurve ℚ :=
  ⟨0, 0, 0, 0, -908800736629952526116772283648363⟩

/-- ICARM leaderboard curve 62 has Mordell-Weil rank at least `17`. -/
public theorem curve062_hasRankGE_17 : HasRankGE curve062 17 := by
  unfold curve062
  certify_curve torsion 7 "data/curve062.txt" "data/curve062-labels.txt"

/-- Curve 62 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve062.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 62. -/
public theorem curve062_j : curve062.j = 0 := j_eq_iff.mpr (by decide +kernel)
