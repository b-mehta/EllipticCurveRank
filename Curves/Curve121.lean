...
transported to the integral short model, are in `data/curve121.txt`; descent labels are in
`data/curve121-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
...
public theorem curve121_j : curve121.j = -5454601499184574464 / 4072172237675 := j_eq_iff.mpr (by decide +kernel)
