...
@[expose] public def curve080 : WeierstrassCurve ℚ := ⟨0, 0, 1, -402157, 96291336⟩

/-- ICARM leaderboard curve 80 has Mordell-Weil rank at least `9`. -/
public theorem curve080_hasRankGE_9 : HasRankGE curve080 9 := by
  unfold curve080
  certify_curve torsion 7 "data/curve080.txt" "data/curve080-labels.txt"

/-- Curve 80 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve080.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 80. -/
public theorem curve080_j : curve080.j = 7193009097905050054656 / 157107745029925477 :=
