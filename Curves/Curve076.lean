...
@[expose] public def curve076 : WeierstrassCurve ℚ := ⟨0, 0, 1, -23737, 960366⟩

/-- ICARM leaderboard curve 76 has Mordell-Weil rank at least `8`. -/
public theorem curve076_hasRankGE_8 : HasRankGE curve076 8 := by
  unfold curve076
  certify_curve torsion 7 "data/curve076.txt" "data/curve076-labels.txt"

/-- Curve 76 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve076.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 76. -/
public theorem curve076_j : curve076.j = 1479112480222949376 / 457532830151317 :=
