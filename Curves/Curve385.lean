...
transported to the integral short model, are in `data/curve385.txt`; descent labels are in
`data/curve385-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
