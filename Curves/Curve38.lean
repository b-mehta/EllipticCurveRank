...
transported to the integral short model, are in `data/curve38.txt`; descent labels are in
`data/curve38-labels.txt`. The `certify_curve` tactic kernel-checks the resulting certificate.
-/
...
@[expose] public def curve38 : WeierstrassCurve ℚ :=
...
public theorem curve38_hasRankGE_18 : HasRankGE curve38 18 := by
  unfold curve38
  certify_curve oneTorsion 12732161191102245007949048 5 "data/curve38.txt" "data/curve38-labels.txt"

/-- Curve 38 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve38.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

set_option linter.style.longLine false in
/-- The `j`-invariant of curve 38. -/
public theorem curve38_j : curve38.j = 13323059362590002593299168845444130391705189385152573903874739428631849797263905708539525584729853042496428109120258334733841162904613294558121181493 / 142169832701772977718561293984707384280114726482259092666685743596875256155267836859249376913473187810947611641871603576022969816446693834489856 :=
