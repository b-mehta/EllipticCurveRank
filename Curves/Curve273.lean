...
over `ℚ`. It has Mordell-Weil rank at least `30`. Submitted to the leaderboard by ranksunbounded.
-/
...
@[expose] public def curve273 : WeierstrassCurve ℚ :=
...
public theorem curve273_hasRankGE_30 : HasRankGE curve273 30 := by
  unfold curve273
  certify_curve torsion 23 "data/curve273.txt" "data/curve273-labels.txt"

/-- Curve 273 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve273.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

set_option linter.style.longLine false in
/-- The `j`-invariant of curve 273. -/
public theorem curve273_j : curve273.j = -908421207611619966397301371501291919756761922734684482806268125472094471799504840865963593736467876552674219212405145331405042867323019584013417593982615086603074757004166961559271290889612721 / 46714661255308767314567688733841531918983356002159772613256840842851650254036518701100578342601553513579222272710220496887616034526983492843954090554197033638137245037791044053017600000000 :=
