...
@[expose] public def curve071 : WeierstrassCurve ℚ := ⟨0, -1, 0, -75, -134⟩

/-- ICARM leaderboard curve 71 has Mordell-Weil rank at least `4`. -/
public theorem curve071_hasRankGE_4 : HasRankGE curve071 4 := by
  unfold curve071
  certify_curve torsion 17 "data/curve071.txt" "data/curve071-labels.txt"

/-- Curve 71 is elliptic (nonzero discriminant), so its `j`-invariant is defined. -/
public instance : curve071.IsElliptic := isElliptic_of_Δ_ne_zero (by decide +kernel)

/-- The `j`-invariant of curve 71. -/
public theorem curve071_j : curve071.j = 2955053056 / 1026877 := j_eq_iff.mpr (by decide +kernel)
